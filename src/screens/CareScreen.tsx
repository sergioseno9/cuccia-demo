...
export function CareScreen() {
  ...
  return <div className="screen care-screen">
    ...
    <section className="care-index"><h2>Il libretto</h2><div className="care-index-card">{indexItems.map(({ icon: Icon, ...item }) => <button id={item.section === 'vaccination' ? 'tutorial-care-add' : undefined} key={item.section} onClick={() => setSection(item.section)}><Icon className={`tone-${item.tone}`} size={23} /><strong>{item.label}</strong><span>{item.value}</span><ChevronRight size={20} /></button>)}</div></section>
