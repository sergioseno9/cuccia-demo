import { AlertCircle, Check, ChevronRight, HeartPulse, Pill, Plus, Scale, Scissors, Shield, ShieldCheck, Stethoscope, Syringe } from 'lucide-react'
...
export function CareScreen() {
  ...
  const grooming = [...data.health.grooming].sort((a, b) => b.lastDate.localeCompare(a.lastDate))
  ...
  return (
    <div className="screen health-screen care-screen">
      ...
      <section id="vaccination" className="health-section"><SectionTitle icon={Syringe} title="Vaccinazioni" tutorialId="tutorial-care-add" onAdd={() => setDialog('vaccination')} />{data.health.vaccinations.length ? <div className="record-list">{data.health.vaccinations.map((record) => <article className="record-row" key={record.id}><div className="record-marker"><Check size={18} /></div><div><strong>{record.name}</strong><p>Somministrato {formatDate(record.administeredDate)}</p>{record.notes && <small>{record.notes}</small>}</div><span>Prossimo<br/><strong>{record.nextDate ? formatDate(record.nextDate) : 'da definire'}</strong></span></article>)}</div> : <div className="empty-inline">Nessuna vaccinazione inserita.</div>}</section>
      ...
      <section id="medication" className="health-section medication-section"><SectionTitle icon={Pill} title="Farmaci e terapie" onAdd={() => setDialog('medication')} />{data.health.medications.length ? <div className="therapy-list">{data.health.medications.map((record) => { const doses = data.events.filter((event) => !event.deletedAt && event.type === 'medication' && event.medicationId === record.id).slice(0, 3); return <article className="therapy-card" key={record.id}><div className="therapy-main"><div><span className="badge badge-sage">{record.active ? 'In corso' : 'Conclusa'}</span><h3>{record.name}</h3><p>{record.dose} · {record.times.join(' / ') || 'orario libero'}</p></div></div><div className="dose-history"><strong>Ultime dosi registrate nel Diario</strong>{doses.length ? doses.map((dose) => { const author = profile.caregivers.find((caregiver) => caregiver.id === dose.caregiverId); return <span key={dose.id}>{author?.name ?? 'Caregiver'} · {timeFormatter.format(new Date(dose.happenedAt))}</span> }) : <span>Ancora nessuna dose registrata.</span>}</div></article> })}</div> : <div className="empty-inline">Nessuna terapia inserita.</div>}</section>
