...
export function HealthRecordDialog({ type, onClose }: { type: HealthRecordType; onClose: () => void }) {
  ...
  const submit = () => {
    ...
    if (type === 'grooming') addGrooming({ title: name, lastDate: date, intervalWeeks: Number(interval) || 0, notes: secondary })
    onClose()
  }
  ...
  return (
    <Modal title={titles[type]} onClose={onClose}>
      ...
      <div className="form-stack">
        <label className="field"><span>{type === 'weight' ? 'Peso in kg' : type === 'prevention' || type === 'deworming' ? 'Prodotto' : type === 'visit' ? 'Motivo' : type === 'grooming' ? 'Cosa è stato fatto' : 'Nome'}</span><input type={type === 'weight' ? 'number' : 'text'} step={type === 'weight' ? '0.1' : undefined} value={name} onChange={(event) => setName(event.target.value)} placeholder={type === 'weight' ? '7,4' : type === 'grooming' ? 'Es. Bagno e spazzolatura' : ''} autoFocus /></label>
        ...
        <label className="field"><span>{type === 'visit' ? 'Data appuntamento' : type === 'prevention' || type === 'deworming' ? 'Ultima somministrazione' : type === 'weight' ? 'Data misurazione' : type === 'medication' ? 'Inizio terapia' : type === 'grooming' ? 'Ultima volta' : 'Data somministrazione'}</span><input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label>
        ...
        {type === 'grooming' && <label className="field"><span>Promemoria morbido <small>opzionale</small></span><select value={interval} onChange={(event) => setInterval(event.target.value)}><option value="0">Nessun promemoria</option><option value="2">Ogni ~2 settimane</option><option value="4">Ogni ~4 settimane</option><option value="6">Ogni ~6 settimane</option><option value="8">Ogni ~8 settimane</option></select><small>È solo un’abitudine indicativa, non una scadenza.</small></label>}
      </div>
