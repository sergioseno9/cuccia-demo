...
export function DocumentManager() {
  ...
  return (
    <section className="profile-section documents-section">
      ...
      <div className="document-add"><select value={kind} onChange={(event) => setKind(event.target.value as PetDocumentKind)}>{Object.entries(kindLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><label className="button-secondary"><Plus size={16} /> Aggiungi file<input type="file" accept="image/*,.pdf" onChange={(event) => addDocument(event.target.files?.[0])} /></label></div>
